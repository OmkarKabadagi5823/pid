...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  node_priv.param<double>("windup_lower_limit", windup_lower_limit_, 0.0);
  node_priv.param<double>("short_limit", short_limit_, 0.0);
...
void PidObject::reconfigureCallback(pid::PidConfig& config, uint32_t level)
{
  ...
  Kp_ = config.Kp * config.Kp_scale * 50;
  Ki_ = config.Ki * config.Ki_scale * 50;
  Kd_ = config.Kd * config.Kd_scale * 50;
...
void PidObject::doCalcs()
{
  ...
  if (new_state_or_setpt_)
  {
    ...
    //Apply short limit
    if((Ki_*fabsf(error_integral_))<fabsf(short_limit_) && (Ki_*fabsf(prev_error_integral_))>fabsf(short_limit_))
      error_integral_ = 0;
    ...
    //Apply windup lower limits
    if (fabs(integral_) < fabsf(windup_lower_limit_))
    {
      integral_ = 0;
    }
    ...
    prev_error_integral_ = error_integral_;
